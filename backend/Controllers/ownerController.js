...
const ownerController = {
    ...
    createArena: async (req, res) => {
        try {
            const {
                name, description, location_lat, location_lng, address,
                base_price_per_hour, sports, time_slots
            } = req.body;
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0)`,
                    [req.user.id, name, description, location_lat, location_lng,
                        address, base_price_per_hour]
                );
                ...
                if (sports && sports.length > 0) {
                    for (const sport of sports) {
                        await connection.execute(
                            ...
                            [arena_id, sport.sport_id, sport.price_per_hour || base_price_per_hour]
                        );
                    }
                }
               ...
               VALUES (?, ?, ?, ?, ?, ?, TRUE)`,
                            [arena_id, slot.sport_id || null, slot.date, slot.start_time,
                                slot.end_time, slot.price || base_price_per_hour]
                        );
                    }
                }
    ...
    updateArena: async (req, res) => {
        try {
            ...
            const { name, description, address, base_price_per_hour, is_active } = req.body;
        ...
        try {
            ...
            try {
               ...
               WHERE arena_id = ? AND date = ? AND start_time = ? AND end_time = ?`,
                 ...
                 WHERE slot_id = ?`,
                                [is_blocked || false, is_holiday || false, slot.price, existingSlot[0].slot_id]
                            );
                        } else {
                            // Create new slot
                            await connection.execute(
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                                [arena_id, date, slot.start_time, slot.end_time,
                                    slot.price, is_blocked || false, is_holiday || false]
                            );
                        }
                    }
        ...
        try {
            ...
            switch (period) {
                ...
                case 'week':
                    ...
                    break;
                case 'month':
                    dateFilter = 'MONTH(booking_date) = MONTH(CURDATE()) AND YEAR(booking_date) = YEAR(CURDATE())';
                    break;
                case 'year':
    ...
    addManager: async (req, res) => {
        try {
         ...
         VALUES (?, ?, ?, ?, ?, ?)`,
                [req.user.id, name, email, hashedPassword, phone_number, JSON.stringify(permissions)]
            );
