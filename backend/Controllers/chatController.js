    ...
    sendMessage: async (req, res) => {
        try {
           ...
           WHERE b.booking_id = ? AND a.owner_id = ?`,
            ...
            // Check for contact information in message (basic check)
            const containsContactInfo = checkForContactInfo(message);
         ...
         VALUES (?, ?, ?, ?, ?)`,
                [booking_id, sender_id, sender_type, message.trim(), containsContactInfo]
            );
...
// Helper function to check for contact information in messages
function checkForContactInfo(message) {
    const contactPatterns = [
        /\d{10,}/, // Phone numbers (10+ digits)
        /@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}/, // Email addresses
        /(http|https):\/\/[^\s]+/, // URLs
        /(\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/, // Various phone formats
    ];
