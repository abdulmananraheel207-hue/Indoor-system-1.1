...
const CommissionPayments = () => {
  ...
  useEffect(() => {
    const loadPayments = async () => {
      try {
        ...
        const normalized = (data.pending_commissions || []).map((p) => ({
          ...p,
  ...
  const filteredPayments = payments.filter((payment) => {
    const matchesSearch =
      payment.arena_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const confirmMarkPaid = async () => {
    if (selectedPayment) {
      try {
        await integrationService.markArenaPayment(selectedPayment.arena_id, {
          amount_paid: selectedPayment.amount_due,
          payment_date: new Date().toISOString().split("T")[0],
        });
        setPayments(
          payments.map((p) =>
            p.arena_id === selectedPayment.arena_id
              ? {
                  ...p,
                  status: "paid",
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600">
          Manage and track commission payments from arena owners
        </p>
      </div>
                    ...
                    <div className="space-y-1">
                      <div className="text-sm">
                        ...
                        <span className="font-medium text-gray-900">
                          Rs {payment.amount_due.toLocaleString()}
                        </span>
                      </div>
                      <div className="text-sm">
                        <span className="text-gray-600">Paid:</span>{" "}
                        <span className="font-medium text-green-600">
                          Rs {payment.amount_paid.toLocaleString()}
                        </span>
                      </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    Rs {selectedPayment.amount_due.toLocaleString()}
                  </p>
                </div>
                <div>
