// components/manager/ManagerArenaSettings.jsx - FIXED VERSION
...
const ManagerArenaSettings = ({ permissions }) => {
        ...
        return (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
                ...
                <p className="text-yellow-600 mt-2">
                    You need view_arena permission to access this section
                </p>
            </div>
        );
    }
    ...
    useEffect(() => {
        if (selectedArena && canManageArena) {
            fetchCourts();
        }
    }, [selectedArena]);
        ...
        try {
            ...
            if (response.ok) {
                ...
                const processedCourts = data.map(court => ({
                    ...court,
                    // Convert sports_names to array if it's a string
                    sports_names: court.sports_names
                        ? (Array.isArray(court.sports_names)
                            ? court.sports_names
                            : court.sports_names.split(',').map(s => s.trim()))
                        : [],
                    // Ensure sports array exists
                    sports: court.sports || [],
                    // Ensure images array exists
                    images: court.images || [],
                    // Ensure court_number exists
                    court_number: court.court_number || 1,
                    // Ensure court_name exists
                    court_name: court.court_name || `Court ${court.court_number || 1}`,
                    // Ensure size_sqft exists
                    size_sqft: court.size_sqft || 2000,
                    // Ensure price_per_hour exists
                    price_per_hour: court.price_per_hour || 500,
    ...
    const handleCourtEdit = (court) => {
        if (!canManageArena) {
            alert("You don't have permission to edit courts");
            return;
        }
    ...
    const handleCourtUpdate = async () => {
        if (!editCourt || !canManageArena) return;
        ...
        try {
            ...
            if (response.ok) {
                ...
                const data = await response.json();
                alert(data.message || "Failed to update court details");
            }
        } catch (error) {
            console.error("Error updating court:", error);
            alert("An error occurred while updating court");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddCourt = async () => {
        ...
        if (!newCourtForm.court_name || !newCourtForm.size_sqft || !newCourtForm.price_per_hour) {
        ...
        try {
            ...
            // FIX: Use the correct endpoint - should be /api/managers/arenas/:arena_id/courts
            // But since we don't have that in managers.js yet, we'll use the owner endpoint
            const response = await fetch(
                `http://localhost:5000/api/owners/arenas/${selectedArena}/courts`,
                {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            alert("An error occurred while adding court");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDeletePhoto = async (courtId, photo) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(
                `http://localhost:5000/api/owners/courts/${courtId}/photos/${photo.image_id}`,
                {
                    method: "DELETE",
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    // FIX: Safe function to render sports names
    const renderSportsNames = (sportsNames) => {
        if (!sportsNames) return null;

        // If it's already an array, use it
        if (Array.isArray(sportsNames)) {
            return sportsNames.map((sport, index) => (
                <span
                    key={index}
        ...
        if (typeof sportsNames === 'string') {
            return sportsNames.split(',').map((sport, index) => (
                <span
                    key={index}
                    className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded"
                >
    ...
    return (
        <div className="max-w-6xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden">
                <div className="p-4 border-b bg-gray-50 md:p-6">
                    ...
                    <p className="text-sm text-gray-600">
                        {!canManageArena && "View-only mode - Contact owner for changes"}
                        {canManageArena && "Edit details and upload photos for each court"}
                    </p>
                </div>
                ...
                <div className="p-4 md:p-6">
                    {loading ? (
                        <div className="text-center py-12">
                            ...
                            <p className="mt-2 text-sm text-gray-600">Loading courts...</p>
                        </div>
                    ) : courts.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            {courts.map((court) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* PHOTO SECTION - Only if manage_arena */}
                                        {canManageArena && (
            ...
            {/* Edit Court Modal - Only if manage_arena */}
            {editCourt && canManageArena && (
            ...
            {/* Add Court Modal - Only if manage_arena */}
            {showAddCourt && canManageArena && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
