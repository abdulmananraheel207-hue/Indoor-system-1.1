// components/manager/ManagerCalendar.jsx - UPDATED TO MATCH OWNER CALENDAR
...
const ManagerCalendar = ({ arenas = [], permissions = {} }) => {
    ...
    // Add new slot form state - Only if manage_calendar permission
    ...
    // Permission checks
    const canViewCalendar = permissions?.view_calendar;
    const canManageCalendar = permissions?.manage_calendar;

    // If no permission to view calendar at all
        ...
        return (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
                ...
                <p className="text-yellow-600 mt-2">
                    You need view_calendar permission to access this section
                </p>
            </div>
        );
    }
    ...
    useEffect(() => {
        if (selectedArena && canManageCalendar) {
            fetchCourts();
        }
    }, [selectedArena]);
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data && data.length > 0) {
                    ...
                    setSelectedCourt(data[0].court_id);
                } else {
                    setCourts([]);
                    setSelectedCourt("");
                }
    ...
    const fetchTimeSlots = async () => {
        try {
            ...
            // Use manager endpoint
            const response = await fetch(
                `http://localhost:5000/api/managers/calendar?arena_id=${selectedArena}&date=${dateStr}${selectedCourt ? `&court_id=${selectedCourt}` : ''}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
    ...
    const handleAddSlot = () => {
        if (!canManageCalendar) {
            alert("You don't have permission to add slots");
            return;
        }
    ...
    const handleEditSlot = (slot) => {
        if (!canManageCalendar) {
            alert("You don't have permission to edit slots");
            return;
        }
    ...
    const handleUpdateSlot = () => {
        if (!editingSlot || !canManageCalendar) return;
    ...
    const handleDeleteSlot = (slotId) => {
        if (!canManageCalendar) {
            alert("You don't have permission to delete slots");
            return;
        }
    ...
    const handleBlockAll = async () => {
        if (!canManageCalendar) {
            alert("You don't have permission to block slots");
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                setTimeSlots(updatedSlots);
                setSuccessMessage("All slots blocked successfully!");
                setTimeout(() => setSuccessMessage(""), 3000);
            } else {
                const data = await response.json();
                alert(data.message || "Failed to block slots");
            }
        } catch (error) {
            console.error("Error blocking slots:", error);
            alert("An error occurred while blocking slots");
        }
    };

    const handleUnblockAll = async () => {
        if (!canManageCalendar) {
            alert("You don't have permission to unblock slots");
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                setTimeSlots(updatedSlots);
                setSuccessMessage("All slots unblocked successfully!");
                setTimeout(() => setSuccessMessage(""), 3000);
            } else {
                const data = await response.json();
                alert(data.message || "Failed to unblock slots");
            }
        } catch (error) {
            console.error("Error unblocking slots:", error);
            alert("An error occurred while unblocking slots");
        }
    };
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const data = await response.json();
                alert(data.message || "Failed to save time slots");
            }
        } catch (error) {
            console.error("Error saving time slots:", error);
            alert("An error occurred while saving time slots");
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            disabled={!selectedArena || courts.length === 0 || !canManageCalendar}
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-xs text-gray-500">
                                    ...
                                    {!canManageCalendar && " • View-only mode"}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden mb-6">
                ...
                {/* Add New Slot Form - Only if manage_calendar */}
                ...
                {/* Add Slot Button - Only if manage_calendar */}
                {canManageCalendar && (
            ...
            {/* Edit Slot Modal - Only if manage_calendar */}
            {editingSlot && canManageCalendar && (
